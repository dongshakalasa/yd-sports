<template>
  <div>
    ...
    <el-table style="width: 100%" border :data="infoList">
      ...
      <el-table-column prop="tmt_time" label="注册时间"></el-table-column>
      <el-table-column label="账号状态">
        <template slot-scope="{row}">
          <div v-if="row.tmt_state == 0" style="color: green">正常</div>
          <div v-else style="color: red">冻结</div>
        </template>
      </el-table-column>
...
export default {
  ...
  methods:{
    async getData(){
      ...
      const result = await this.$API.reqMerchantInfo(page, limit);
      if (result.code == 200) {
